# ============================================================================
# Deploy Compliance Reporting Watchlists to Microsoft Sentinel
# ============================================================================
...
foreach ($watchlist in $watchlists) {
    ...
    # Save schema to temporary CSV file
    $tempFile = [System.IO.Path]::GetTempFileName() + ".csv"
    $watchlist.Schema | Out-File -FilePath $tempFile -Encoding UTF8

    try {
        # Check if watchlist already exists
        $existingWatchlist = Get-AzSentinelWatchlist `
            -ResourceGroupName $ResourceGroupName `
            -WorkspaceName $WorkspaceName `
            -WatchlistAlias $watchlist.Name `
            -ErrorAction SilentlyContinue

        if ($existingWatchlist) {
            Write-Host "Watchlist $($watchlist.Name) already exists. Updating..." -ForegroundColor Cyan

            # Update existing watchlist
            Update-AzSentinelWatchlist `
                -ResourceGroupName $ResourceGroupName `
                -WorkspaceName $WorkspaceName `
                -WatchlistAlias $watchlist.Name `
            ...
            Write-Host "Creating new watchlist $($watchlist.Name)..." -ForegroundColor Cyan

            # Create new watchlist
            New-AzSentinelWatchlist `
                -ResourceGroupName $ResourceGroupName `
                -WorkspaceName $WorkspaceName `
                -WatchlistAlias $watchlist.Name `
                -DisplayName $watchlist.DisplayName `
            ...
            New-AzSentinelWatchlistItem `
                -ResourceGroupName $ResourceGroupName `
                -WorkspaceName $WorkspaceName `
                -WatchlistAlias $watchlist.Name `
                -ItemsKeyValue $tempFile
        }

        Write-Host "Successfully created/updated watchlist: $($watchlist.Name)" -ForegroundColor Green
    }
    catch {
        Write-Host "Error creating watchlist $($watchlist.Name): $_" -ForegroundColor Red
    }
    finally {
        # Clean up temp file
...
# Display summary
Write-Host "`nCreated/Updated Watchlists:" -ForegroundColor Yellow
$watchlists | ForEach-Object {
